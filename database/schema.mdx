...
| Table | Key columns |
|-------|-------------|
| `ai_employees` | PK `(workspace_id, id)`, `role_key`, `provider`, `model_mode`, `participation_style`, `permissions` |
...
| Table | Key columns |
|-------|-------------|
| `project_rooms` | PK `(workspace_id, id)`, `kind` (`channel`|`dm`), `dm_employee_id` — constrained by `project_rooms_kind_shape` |
...
| `messages` | `room_id`, `topic_id`, `sender_type`, `mentions_json`, `agent_run_id`, `artifacts` |
...
| `agent_runs` | `status`, `employee_id`, `topic_id`, `trigger_message_id`, `response_reason`, `root_trigger_message_id`, `depends_on_run_id`, `run_metadata` |
